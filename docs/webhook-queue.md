# Asynchronous Square Webhook Queue

## Overview

- `/api/square/webhooks` validates the Square signature, snapshots the **raw JSON body**, and enqueues the event into Upstash Redis in <500 ms. When Redis is unavailable (e.g., local dev), it falls back to synchronous processing to keep the workflow unblocked.
...
- Both endpoints run in the Node.js runtime so they can execute heavier logic (database + Square API calls) without the Edge constraints.
...
| `SQUARE_APPLICATION_ID` | Namespaces the Redis keys (`square-webhook-queue:{app_id}`). |
...
- Queue: `square-webhook-queue:{SQUARE_APPLICATION_ID | 'default'}` (FIFO via `LPUSH` + `RPOP`)
- DLQ: `square-webhook-queue-dlq:{SQUARE_APPLICATION_ID | 'default'}`
...
Always confirm that downstream logic is idempotent before replaying—processing the same Square event twice should be a no-op.

## Testing Checklist

- `npm run test __tests__/app/api/square/webhooks.test.ts` – validates signature handling + enqueue behavior.
...
- Manual smoke: `curl -X POST /api/webhooks/process -H "Authorization: Bearer $WEBHOOK_PROCESS_TOKEN"` and monitor the logs (`[SquareWebhookWorker]`) for processed counts.
