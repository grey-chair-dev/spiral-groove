...
const databaseUrl = process.env.SGR_DATABASE_URL || process.env.DATABASE_URL;
...
2. Update `.env.local`:
   ```env
   SGR_DATABASE_URL=postgresql://app_user:password@pooler-host/database?sslmode=require&pgbouncer=true
   ```
...
- Connection string: `SGR_DATABASE_URL` (with pooling)
