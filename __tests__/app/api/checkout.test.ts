...
vi.mock('@/lib/square/catalog', () => mockCatalogModule);
vi.mock('@/lib/square/orders', () => mockOrdersModule);
...
const mockRequireSquareConfig = mockClientModule.requireSquareConfig as vi.Mock;
...
describe('POST /api/checkout', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockRequireSquareConfig.mockReturnValue(undefined);
    mockGetClient.mockReturnValue({
      checkout: {
        paymentLinks: {
          create: vi.fn().mockResolvedValue({
            paymentLink: { url: 'https://square.link/u/test' },
          }),
        },
      },
    });
  ...
  it('uses Square catalog price when creating an order', async () => {
