...
if (action !== 'add' && action !== 'delete') {
  console.error('Usage: node scripts/wishlist-toggle.mjs <add|delete>')
  process.exit(1)
}

const envKey = 'VITE_ENABLE_WISHLIST'
...
if (action === 'add') {
  ...
  if (hasKey && currentValue === 'true') {
    console.log('ℹ️  Wishlist feature is already enabled.')
    process.exit(0)
  }
  ...
  console.log('✅ Wishlist feature enabled (set to true).')
} else {
  // delete action - set to false
  if (hasKey && currentValue === 'false') {
    console.log('ℹ️  Wishlist feature is already disabled.')
    process.exit(0)
  }
  ...
  writeFileSync(envPath, updatedLines.filter(Boolean).join('\n') + '\n')
  console.log('🧹 Wishlist feature disabled (set to false).')
}
