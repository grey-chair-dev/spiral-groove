...
export async function fetchSquareInventory(
  ...
  try {
    ...
    for (const catalogObjectId of catalogObjectIds) {
      try {
        const response = await inventoryApi.retrieveInventoryCount(
          catalogObjectId,
          locationId,
        )
        ...
        console.warn(
          `[SquareAdapter] Error fetching inventory for ${catalogObjectId}:`,
          error,
        )
        inventoryMap.set(catalogObjectId, 0)
      }
...
export async function fetchSquareProducts(
  ...
  try {
    ...
    const inventoryMap = await fetchSquareInventory(
      client,
      catalogObjectIds,
      locationId,
    )
