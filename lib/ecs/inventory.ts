...
import type { ECSInventoryCount } from '@/lib/types/ecs';
...
export async function getInventoryCount(
  ...
  try {
    ...
    const page = await withECSRetry(() =>
      client.inventory.batchGetCounts({
        catalogObjectIds: [catalogObjectId],
      })
    );

    const counts: ECSInventoryCount[] = [];
    for await (const item of page) {
      counts.push(item as ECSInventoryCount);
    }
